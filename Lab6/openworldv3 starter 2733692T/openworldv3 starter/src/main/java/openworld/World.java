...
public class World {
    ...
    public void run() {
        int turnTimer = 1;
        Coordinates winCondition = new Coordinates(7, 7);
        while (!gameOver) {
            System.out.println("\n\n-------------------------------");
            ...
            turnTimer++;
            if ((this.getAdventurer().getLocation().getX() == winCondition.getX()
                    && this.getAdventurer().getLocation().getY() == winCondition.getY())
                    || getAdventurer().getCurrentHealth() <= 0) {
                setGameOver(true);
            }
        }
            ...
            for (int y = 0; y <= yDimension; y++) {
                ...
                for (Items i : item) {
                    if (i.getLocation().equals(currentLocation)) {
                        entities.add(getShortenedName(i.getName()));
                    }
                }
                if (!entities.isEmpty()) {
    ...
    public void resolveMove(TravellingWorldEntity traveller) {
        ...
        ArrayList<Items> itemsHere = getItemsHere(traveller.getLocation());
        for (Items i : itemsHere) {
            if (i.equals(traveller)) {

            } else if (i instanceof Wand) {
                i = (Wand) i;
                i.encounter(traveller);
        ...
        for (NPC npc : charactersHere) {
            if (npc.equals(traveller)) {
                // do nothing
            } else if (npc instanceof Wizard && adventurer.getSpellBoolean() == true) {
                npc = (Wizard) npc;
                npc.encounter(traveller);
                adventurer.setSpellBoolean(false);
            } else if (npc instanceof Healer && adventurer.getHealBoolean() == true) {
                npc = (Healer) npc;
                npc.encounter(traveller);
                adventurer.setHealBoolean(false);
            }
        }
    }
    ...
    private ArrayList<Items> getItemsHere(Coordinates location) {
        ArrayList<Items> itemsHere = new ArrayList<>();
        for (Items i : item) {
    ...
    public static void main(String[] args) {
        ...
        w.initaliseWorld();
        w.nonPlayerCharacters.add(new Healer("Sage", new Coordinates(0, 0), 15, w, new Damage(15, DamageType.PHYSICAL),
                w.randomCoordinates()));
        w.run();
    ...
    public void generateItems() {
        ...
        for (int x = 0; x <= xDimension; x++) {
            for (int y = 0; y <= yDimension; y++) {
                countUp++;
                if (countUp == frequency) {
                    if (wandCount <= potionCount) {
                        item.add(new Wand("Wand" + wandCount, new Coordinates(x, y), 100, this,
                                new Damage(80, DamageType.PHYSICAL)));
                        wandCount++;
                        countUp = 0;
                    } else {
                        item.add(new Potion("Potion" + potionCount, new Coordinates(x, y), 75, this,
                                new Damage(0, DamageType.PHYSICAL)));
                        potionCount++;
                        countUp = 0;
                    }
